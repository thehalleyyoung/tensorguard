...
program edits, which is the wrong
...
multilinear-algebra libraries.
Our two lemmas (\S\ref{sec:lean})
...
benchmarks. Failure-mode catalogues from agent-tooling vendors
\citep{langchainfm} list ``silent runtime errors''
...
{\scriptsize\verb|self.proj = nn.Linear(d, h);|}\\
{\scriptsize\verb|self.proj(x.transpose(-1,-2))|}
...
it matches the qualitative claims of agent-failure catalogues
\citep{langchainfm}
...
\label{sec:artifact}
All claims in this paper are reproduced by a single self-checking
\emph{reproducibility capsule} shipped with the source release. From a
checkout, one command
%
\begin{center}
\texttt{bash capsule/reproduce.sh}
\end{center}
%
(i) verifies the environment against the pinned lockfile
(\texttt{capsule/requirements.lock.txt}), (ii) regenerates every
deterministic artifact from source in dependency order, asserting
byte-identical determinism, and (iii) re-audits every numeric claim in the
project \texttt{README} by recomputing each ratio and percentage from the
freshly regenerated files. The harness exits non-zero on the first mismatch,
so a green run \emph{is} the reproduction. The six-snippet gradient-flow
corpus of Section~\ref{sec:corpus}, the three-way diagnostic comparison of
Section~\ref{sec:headtohead}, and the verified rewrite rules of
Section~\ref{sec:lean} are each regenerated and validated by this capsule;
the claim$\to$command map and badge justification are in
\texttt{docs/artifact/} (Artifact Appendix, \texttt{INSTALL.md},
\texttt{REQUIREMENTS.md}, \texttt{STATUS.md}). The wiring between this
submission and the capsule is itself checked in CI
(\texttt{tests/test\_paper\_capsule\_wiring.py}), so the paper cannot drift
from the artifact it cites.

\bibliographystyle{icml2024}
\begin{thebibliography}{9}
\small
\bibitem[Jhoo et al.(2022)]{pytea}
H.~Y.~Jhoo, S.~Kim, W.~Song, K.~Park, D.~Lee, and K.~Yi.
\newblock A Static Analyzer for Detecting Tensor Shape Errors in Deep Neural
Network Training Code.
\newblock In \emph{ICSE Companion}, 2022.
...
M.~Hattori, N.~Kobayashi, and R.~Sato.
\newblock Gradual Tensor Shape Checking.
\newblock In \emph{ESOP}, 2023.
...
\bibitem[Xia et al.(2023)]{verifiedrepair}
...
\newblock In \emph{APR Workshop at ICSE}, 2023.
\end{thebibliography}
