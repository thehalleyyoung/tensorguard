...
# ---------------------------------------------------------------------------
# Tensor Shape Guard Extractor
# ---------------------------------------------------------------------------

class TensorShapeGuardExtractor(ast.NodeVisitor):
    """Extract shape-related guards from tensor operations.

    Recognises patterns like:
    - x.view(...), x.reshape(...) — reshape constraints
    - x.permute(...), x.transpose(...) — dimension reordering
    - torch.cat([a, b], dim=...) — concatenation constraints
    - torch.stack([a, b], dim=...) — stacking constraints
    - x.expand(...), x.repeat(...) — expansion constraints
    - F.pad(x, ...) — padding constraints
    - torch.einsum('...', ...) — dimension constraints
    - x.contiguous().view(...) — chains
    - assert x.shape[0] == y.shape[0] — shape assertions
    """
    ...
    def visit_Assert(self, node: ast.Assert) -> None:
        self._check_shape_assertion(node)
        self.generic_visit(node)

    def _check_shape_call(self, node: ast.Call) -> None:
        """Detect tensor shape manipulation calls."""
        func = node.func
        if not isinstance(func, ast.Attribute):
            # Check for torch.cat, torch.stack, torch.einsum, F.pad
            func_name = _expr_to_str(func)
            if 'cat' in func_name or 'stack' in func_name:
                self._add_shape_guard(
                    node, "concatenation",
                    f"torch.cat/stack requires compatible shapes along all non-cat dims",
                    [_expr_to_str(a) for a in node.args[:1]],
                )
            elif 'einsum' in func_name:
                if node.args and isinstance(node.args[0], ast.Constant):
                    eq = node.args[0].value
                    self._add_shape_guard(
                        node, "einsum",
                        f"torch.einsum('{eq}') constrains shared indices",
                        [_expr_to_str(a) for a in node.args[1:]],
                    )
            elif 'pad' in func_name:
                self._add_shape_guard(
                    node, "padding",
                    "F.pad changes spatial dimensions by padding amounts",
                    [_expr_to_str(a) for a in node.args[:1]],
                )
            return

        method = func.attr
        base = _expr_to_str(func.value)

        if method in ('view', 'reshape'):
            dims = [_expr_to_str(a) for a in node.args]
            self._add_shape_guard(
                node, "reshape",
                f"{base}.{method}({', '.join(dims)}) requires total elements match",
                [base],
            )
        elif method == 'permute':
            dims = [_expr_to_str(a) for a in node.args]
            self._add_shape_guard(
                node, "permute",
                f"{base}.permute({', '.join(dims)}) reorders dimensions",
                [base],
            )
        elif method == 'transpose':
            dims = [_expr_to_str(a) for a in node.args[:2]]
            self._add_shape_guard(
                node, "transpose",
                f"{base}.transpose({', '.join(dims)}) swaps two dimensions",
                [base],
            )
        elif method == 'expand':
            dims = [_expr_to_str(a) for a in node.args]
            self._add_shape_guard(
                node, "expand",
                f"{base}.expand({', '.join(dims)}) expands singleton dims",
                [base],
            )
        elif method == 'repeat':
            dims = [_expr_to_str(a) for a in node.args]
            self._add_shape_guard(
                node, "repeat",
                f"{base}.repeat({', '.join(dims)}) repeats tensor along dims",
                [base],
            )
        elif method in ('squeeze', 'unsqueeze'):
            dim_arg = _expr_to_str(node.args[0]) if node.args else "None"
            self._add_shape_guard(
                node, method,
                f"{base}.{method}({dim_arg}) changes tensor rank",
                [base],
            )
        elif method == 'flatten':
            start = _expr_to_str(node.args[0]) if node.args else "1"
            self._add_shape_guard(
                node, "flatten",
                f"{base}.flatten({start}) collapses dimensions from start_dim",
                [base],
            )

    def _check_shape_assertion(self, node: ast.Assert) -> None:
        """Detect shape assertions like assert x.shape[0] == y.shape[0]."""
        test = node.test
        if isinstance(test, ast.Compare):
            left = _expr_to_str(test.left)
            if '.shape' in left or '.size' in left:
                comparators = [_expr_to_str(c) for c in test.comparators]
                vars_list = [left] + comparators
                self._add_shape_guard(
                    node, "shape_assertion",
                    f"Shape assertion: {left} vs {', '.join(comparators)}",
                    vars_list,
                )

    def _add_shape_guard(
        self, node: ast.AST, pattern_name: str, message: str,
        variables: List[str],
    ) -> None:
        loc = SourceLocation.from_node(node, self.filename)
        self._guards.append(ExtractedGuard(
            pattern=GuardPattern.Structural,
            variables=variables,
            predicate=ComparisonPredicate(
                left_expr=pattern_name,
                op=ComparisonOp.EQ,
                right_expr=message,
                variables=variables,
                provenance="shape_guard",
            ),
            source_location=loc,
            polarity=Polarity.TRUE_BRANCH,
            raw_source=message,
...
class FullGuardExtractor:
    ...
    def __init__(self, filename: str = "", source: str = "") -> None:
        ...
        self._shape = TensorShapeGuardExtractor(filename)
    ...
    def extract_all(self, tree: ast.AST) -> List[ExtractedGuard]:
        ...
        guards.extend(self._shape.extract(tree))
