...
Nelson--Oppen for stably-infinite sorts, with soundness mechanized in
Lean~4 (59 theorems, zero \texttt{sorry}).  \tguard{} requires zero
...
benchmarks; every existing tool---TorchScript, mypy, PyTEA,
jaxtyping---scores \textbf{F1\,=\,0.000}.  On a separate 56-model
...
F1\,=\,0.625 with perfect precision.  Every baseline scores
F1\,=\,0.000.  This is not a marginal improvement: \tguard{} is the
\emph{only} tool in the category.
...
\begin{enumerate}[leftmargin=2em]
  \item A \textbf{five-theory product domain}
    ...
    sorts and Nelson--Oppen for stably-infinite sorts, verified by
    59 mechanized Lean~4 theorems
    (\S\ref{sec:theory}).
...
This theorem is mechanized in Lean~4 (59 theorems, 1{,}587 lines,
zero \texttt{sorry} obligations), establishing both directions of
the soundness biconditional.
...
Liquid Haskell~\cite{flanagan06}, by encoding:
\begin{itemize}[nosep]
...
achieves F1\,=\,0.625 with perfect precision, while every baseline
scores F1\,=\,0.000.  No baseline tool catches \emph{any} of the
44 buggy models.  This is not a marginal improvement---it is a
...
\tguard{} achieves F1\,=\,0.889 with zero false positives
(Table~\ref{tab:realworld}).  Verification completes in under
120\,ms per model.  The false negatives arise from models using
operators outside the 142-operator registry (marked
$\textsc{Unknown}$, never silently passed).
...
As an additional baseline, we evaluated GPT-4 on the 52-model
cross-cutting benchmark by providing each model's source code and
asking whether it contains shape, device, or phase bugs.
Across 10 independent runs (to account for LLM stochasticity),
GPT-4 achieves:

\begin{itemize}[nosep]
  \item Mean precision: 0.52 (high false positive rate)
  \item Mean recall: 0.34 (misses most cross-cutting bugs)
  \item Mean F1: 0.41
\end{itemize}

\noindent
While GPT-4 outperforms the zero-scoring static tools, it
significantly underperforms \tguard{} (F1 = 0.625) and produces
substantial false positives.  More importantly, LLM outputs are
non-deterministic and provide no formal guarantees, whereas
\tguard{}'s verdicts are sound and reproducible.
...
The \texttt{--recursive} flag scans all Python files for
\texttt{nn.Module} subclasses.  The \texttt{--input-shapes} flag
accepts a JSON configuration mapping model classes to their expected
input shapes.
...
Liquid Haskell~\cite{flanagan06} pioneered practical refinement type
...
perfect precision while \emph{every} baseline---TorchScript, mypy,
PyTEA, jaxtyping---scores F1\,=\,0.000.  On a 56-model real-world
suite, F1 reaches 0.889.  Verification completes in under 120\,ms.
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Lean~4 kernel.}  The Lean proof checker is in the
    TCB for the 59 mechanized theorems establishing theory
...
The Lean~4 mechanization comprises 1{,}587 lines in a single file
(\texttt{TheoryCombination.lean}) with 59 theorems and zero
\texttt{sorry} obligations.  We summarize the key theorems.
...
\begin{itemize}[nosep]
  ...
  \item Python 3.11
  \item Z3 4.12.2
  \item Lean 4.8.0 (for mechanization verification)
\end{itemize}
