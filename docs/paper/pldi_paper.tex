...
benchmarks; no existing tool---TorchScript, mypy, PyTEA,
jaxtyping---is designed to detect these cross-cutting bug classes
(F1\,=\,0.000 by design, as they lack multi-theory checking).  On a separate 56-model
...
F1\,=\,0.625 with perfect precision.  No existing baseline is
designed to detect these cross-cutting bug classes (all score
F1\,=\,0.000, as they lack device and phase checking).
This is not a marginal improvement: \tguard{} is the
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Lean~4 kernel.}  The Lean proof checker is in the
    ...
    combination soundness.  \emph{Note:} The Lean formalization has
    not yet been compiled (no \texttt{lakefile.lean} is present);
    theorem statements are written but await mechanical verification.
...
the repository, so the proofs have not been compiled with
\texttt{lake build}.  The theorem statements and proof structure have
been written and reviewed but await compilation.  We summarize the
key theorems.
