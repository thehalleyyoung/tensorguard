...
\begin{theorem}[Fragment-level soundness]
\label{thm:fragment-soundness}
Let $M\in\mathcal{F}_\mathrm{TG}$ be a closed module and let
$\Gamma\vdash M:\tau$ with $\mathsf{Verify}(M)=\textsc{Verified}$.
Then for every input $x$ with $\sigma\models\mathit{assume}_M(x)$, the
reduction $\langle M(x),\sigma\rangle\to^* v$ does not invoke the
operator-level shape-mismatch exception of any operator in $\mathrm{Cat}$.
\end{theorem}
\begin{proof}[Sketch]
\emph{Preservation}: by induction on the BNF of $\mathcal{F}_\mathrm{TG}$.
Each operator rule (\textsc{T-Matmul}, \textsc{T-View($-1$)},
\textsc{T-Detach}, and the 76 remaining rules in \Cref{app:typing-rules})
produces a uniquely-typed result; the 28 Lean-audited handlers
(Table~\ref{tab:handler-soundness}) are sorry-free at the rule definitions;
the 3 pen-and-paper handlers follow from the operator-level argument in
\Cref{app:soundness}; the 48 tested-only handlers are not covered by
...
forms.
Together, no well-typed term in the fragment raises a shape-mismatch at any
$\mathit{op}\in\mathrm{Cat}$; the claim is restricted to modeled operators.
\end{proof}
...
(c)~absence of dtype errors outside $\mathrm{Cat}$;
(d)~soundness of the analyser \emph{implementation}---only of the rule
table it implements;
(e)~coverage of the 48 tested-only handlers (Table~\ref{tab:handler-soundness}).
...
(ii)~the operator being in $\mathrm{Cat}$ with a Lean-audited or
pen-and-paper soundness proof; (iii)~Z3 returning \textsc{Unsat} correctly
on every discharged obligation.
