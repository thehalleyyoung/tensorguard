...
on the target machine and is honestly excluded.  Every block is
content-addressed by SHA-256.  The \emph{bug corpus} contains $60$
historical PyTorch shape bugs mined from the
\texttt{pytorch/pytorch} issue tracker via $20{+}$ keyword queries
...
do not synthesize, while \texttt{mypy/jaxtyping} and \texttt{beartype}
...
which over-represents categories like \texttt{view/reshape} and
