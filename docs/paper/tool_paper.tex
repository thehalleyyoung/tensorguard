...
dtype, device, phase, gradient-flow, stride/layout, and library-specific
contracts before a forward pass runs, then reports a three-valued verdict:
...
such as \texttt{verify\_einops}, \texttt{verify\_einops\_source}, and
\texttt{verify\_sdpa}.  A typical workflow is:
...
Two high-value library checkers demonstrate the pattern for precision without
unsoundness:
...
\begin{itemize}[leftmargin=2em]
  \item deeper operator coverage for distributions, named tensors, complex
  dtypes, functorch/vmap, sparse tensors, grid sampling, MultiheadAttention,
...
export, einops, and SDPA hazards before execution; it reports precise
