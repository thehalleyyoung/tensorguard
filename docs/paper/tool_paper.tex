...
contracts, complex view/FFT shape-dtype rules, named-axis alignment, and
library-specific contracts before a forward pass runs, then reports a
...
\texttt{verify\_sdpa}, \texttt{verify\_fft},
\texttt{verify\_view\_as\_real}, and \texttt{verify\_align\_to}.  A typical
workflow is:
...
Five high-value library checkers demonstrate the pattern for precision without
unsoundness:
...
\begin{itemize}[leftmargin=2em]
  \item deeper operator coverage for functorch/vmap, sparse tensors, grid
  sampling, MultiheadAttention,
...
export, einops, SDPA, distribution, complex FFT/view, and named-axis hazards
