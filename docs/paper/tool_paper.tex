...
contracts, named-axis alignment, and library-specific contracts before a
forward pass runs, then reports a three-valued verdict:
...
library calls such as \texttt{einops.rearrange} or
\texttt{scaled\_dot\_product\_attention}, or PyTorch named-tensor
\texttt{align\_to}.
...
\texttt{verify\_architecture}, \texttt{verify\_module}, and specialized helpers
such as \texttt{verify\_einops}, \texttt{verify\_einops\_source},
\texttt{verify\_sdpa}, and \texttt{verify\_align\_to}.  A typical workflow is:
...
Four high-value library checkers demonstrate the pattern for precision without
unsoundness:
...
best-paper-award and 1,000-star release.  The most important next directions are
not more marketing metrics; they are higher-value coverage and stronger
evidence:

\begin{itemize}[leftmargin=2em]
  \item deeper operator coverage for named tensors, complex dtypes,
  functorch/vmap, sparse tensors, grid sampling, MultiheadAttention,
  split/chunk, masked selection, nonzero, and exact einsum diagonal semantics;
...
export, einops, SDPA, distribution, and named-axis hazards before execution; it reports precise
