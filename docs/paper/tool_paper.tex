...
packed/unpacked MultiheadAttention q/k/v and mask contracts, geometric sampler
grids, and library-specific contracts before
...
side includes a Lean 4 library whose 496 public theorems are discovered and
...
\begin{enumerate}[leftmargin=2em]
  ...
  SMT encodings; tests discover and axiom-audit all 496 public theorems.
\end{enumerate}
...
cover einops, SDPA, MultiheadAttention, distributions, named tensors, complex
FFT/view contracts, \texttt{vmap} batched-dimension transfer, sparse tensor
layouts, and \texttt{grid\_sample}/\texttt{affine\_grid}.  A typical
workflow is:
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{\texttt{nn.MultiheadAttention}.}
  \path{src/mha_verify.py} models packed and separate q/k/v projections,
  \texttt{kdim}/\texttt{vdim}, \texttt{batch\_first}, 2-D and 3-D attention
  masks, key-padding masks, and attention-weight output shapes.  Nested tensors
  abstain rather than fabricating rectangular dimensions.  Differential tests
  compare module and \texttt{multi\_head\_attention\_forward} calls against real
  PyTorch, and the \texttt{torch.fx} module path carries explicit q/k/v/mask
  roles into the verifier.
\end{itemize}
...
current discovered count is 496 public theorems.
...
Unflatten, BatchNorm, and the 2-D/3-D \texttt{grid\_sample}/\texttt{affine\_grid}
rank and positive-size contracts.
...
\path{reproducibility/artifact_package.py} extends this into a three-mode
fresh-machine package manifest: Docker builds a wheel-only non-root image,
conda renders a \texttt{noarch: python} recipe with an isolated installed
entrypoint test, and source mode starts from a fresh clone plus virtualenv.
All three modes name their evidence replay command, and source mode runs the
real \path{examples/shape_bug.py} smoke test and observes \unsafe{} before any
claim is accepted.
...
those releases reproducible.  The Step 267 manifest ties the release paths to
fresh-machine commands: \path{Dockerfile} and \path{capsule/Dockerfile.reproduce}
for containerized review, \path{conda-recipe/meta.yaml} for conda-build, and a
source checkout plus virtualenv path for reviewers who want to inspect or modify
the code before running \path{reproducibility/reproduce_all.py --check}.
...
\begin{itemize}[leftmargin=2em]
  \item deeper operator coverage for split/chunk, masked selection, nonzero,
  exact einsum diagonal semantics, and recurrent layers;
...
export, einops, SDPA, MultiheadAttention, distribution, complex FFT/view,
named-axis, vmap, sparse-layout, and sampler-grid hazards
...
Is MultiheadAttention q/k/v and mask handling modeled faithfully? &
\path{src/mha_verify.py} &
\path{tests/test_mha_verify.py} \\
...
Is the artifact packaged without hidden local state? &
\path{reproducibility/artifact_package.md},
\path{docs/artifact/PACKAGING.md} &
\path{tests/test_artifact_package.py} \\
\bottomrule
\end{longtable}
...
Library model & MultiheadAttention packed/unpacked q/k/v, mask, key-padding,
and weight-output contracts &
\path{src/mha_verify.py}, \path{tests/test_mha_verify.py} \\
...
gate, three-mode artifact package, and paper-evidence target &
\path{reproducibility/audit_numeric_claims.py},
\path{docs/dashboard/index.html}, \path{reproducibility/artifact_package.md},
\path{scripts/benchmark_delta.py} \\
\bottomrule
\end{longtable}
...
\path{evaluation/fx_trace_success.json} \\
496 public Lean theorems discovered and axiom-audited &
\path{tests/test_lean_whole_pipeline_audit.py} \\
