...
1. The opening framing still overstates the Dynamo result relative to both the theorem and the empirical evidence. In the introduction, “TorchDynamo guards become the runtime shadow of these refinements” and “Abstain ... marks exactly the s...
2. The headline real-world bug-finding case remains weak. On the user-visible 488-block corpus the paper reports `0/488` unconditional RP, and on the unfiltered post-freeze sample the main comparable number is `5/15` versus `2/15` and `3/15...
3. The grad-lattice runtime holdout appears artifact-inconsistent. §4.4 says the analyser returns Refuted-Proof on `8/8` checkpoint-enabled scripts because its detector flips them out of the first-order lattice, but `reproducibility/grad_la...
4. The theorem-to-evidence bridge for Theorem 5 is still limited. The paper’s own strongest end-to-end audit is only 9 CNN blocks without surrogate, 4 transformer blocks still use surrogates, and the larger-module audit reports zero SHAPE/D...
5. The curated 60-bug benchmark seems insufficiently diagnostic of the intended reasoning contribution. The paper states that with operator dispatch disabled the AST-pattern path alone still gets `53/60`, and with AST patterns disabled the ...
