...
1. **C5 wording in the Introduction contradicts the paper's own ablation data.** Contribution C5 (§1) states "on the real corpora, only three knobs—device-consistency, gradient-flow, and low-confidence gating—move verdicts." But the "Real-c...
2. **Pytea per-bug table claimed but absent from the compiled artifact.** Appendix §"Pytea modern-subset matched-pair contingency table" says the 34-row table is "published as a machine-readable artifact in the reproducibility appendix" and...
3. **Inconsistent "unwitnessed" terminology within a single paragraph.** §4.1 says "Zero assume_M is unwitnessed: every CV refutes against at least one realisable caller pattern" and then two sentences later says "The 10 unwitnessed rows de...
4. **Lean footprint covers a small fraction of deployed verdicts.** Handler_scope_per_block reports that only 11/57 Verified verdicts and 25/128 CV verdicts in the primary real-source experiment fall entirely within the Lean-or-pen-paper au...
5. **AST extractor—the component synthesising assume_M—is untrusted and unvalidated against an independent oracle.** The soundness of all 128 CV verdicts depends on correct synthesis of assume_M from class source by the AST extractor, which...
