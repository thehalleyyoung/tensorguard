...
1. **The 0/488 RP gap remains the dominant empirical fact about practical utility.** The paper responds with the 15/488 RP rerun (under an input-shape contract) and the 53/60 curated-corpus number, but the curated corpus was filtered to ≤40...
2. **Mutation-kill rate 7/50 at the analyser level is inadequate validation for a tool making soundness claims.** The paper employs four mutation classes (comparison flip, boolean-op flip, arithmetic-op swap, small-integer constant +1) but ...
3. **The Lean audit proves rules, not the analyser.** The abstract and introduction's prominent mention of Lean 4 mechanisation risks misleading readers: the AST extractor, analyser implementation, backward verifier, and Z3 dispatch are exp...
4. **Cross-family evaluation uses only synthetic/deliberately-injected bugs.** All five decoder families (Qwen2, Mistral, Gemma, Phi-3) contribute zero caught real bugs; each family's "refuted" module is an author-constructed mismatch varia...
5. **The Dynamo-guard correspondence (C4) is labelled a "theorem" in the contributions but is empirically audited on 14 modules, 4 of which use a forward-signature surrogate.** The necessary-direction-only scope is documented, but placing a...
