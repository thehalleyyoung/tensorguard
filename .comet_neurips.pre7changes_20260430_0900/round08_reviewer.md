...
1. \textbf{Mutation testing on load-bearing handlers shows \(0/10\) kill on \texttt{conv2d} and \(0/10\) on \texttt{einsum}} (eval\_v6.tex L1079-1090). The paper attributes this to "corpus-coverage gap, not a handler correctness concern," b...
2. \textbf{Of the \(185\) in-soundness verdicts on the \(488\)-block corpus, only \(36\) (\(11+25\)) touch \emph{only} the Lean-or-pen-paper audited footprint; \(105/185\) touch at least one of the \(44\) tested-only handlers} (eval\_v6.tex...
3. \textbf{The \(N{=}15\) unfiltered post-freeze headline does not separate from baselines under any standard test} (Fisher \(p{=}0.39\) vs.\ FakeTensor, \(p{=}0.68\) vs.\ Pytea; BH-adjusted \(p{=}1.00\) on every pair). The paper reports th...
4. \textbf{The hybrid-mode "complementarity" claim (Tab.~\ref{tab:hybrid-falsify}: TG-only \(20\), FT-only \(5\), zero overlap) is on a \(25\)-block stress set the authors hand-designed} to exercise three knobs (eval\_v6.tex L737-746). This...
5. \textbf{The grad-flag silent-error rate on the \emph{worst-case} construct family is \(2/8 = 25\%\)} on the runtime trainer harness (eval\_v6.tex L1290-1316), not the \(\le 12\%\) "regex-detectable prevalence" ceiling that the paper fore...
6. \textbf{Theorem 5 (Dynamo-guard correspondence)'s end-to-end empirical anchor is \(13\) SHAPE recompile events on \(10\) CNN-type modules} (eval\_v6.tex L966-976), with the \(4\) transformer blocks audited via the documented forward-sign...
