...
1. The Lean composition DSL is still 13 operators wide and the four operators on the Table 3 post-freeze bug paths (`view/reshape` as a non-trivial shape rule, `conv2d`, `einsum`, the symbolic-rank slice of `expand`) are still outside `ag_c...
2. The 105/185 → 103/185 movement is small. The four promoted handlers (flatten / squeeze / unsqueeze / softmax) are the trivial cases (each reduces to `view` or to T-Identity); the load-bearing handler in the LW→RP attribution table — the ...
3. Of the 12 named LW→RP candidate rows in §4.1's Table 3, 10 remain "Predicted RP" rather than measured. The unbind handler exercise is a proof of concept that the falsifiable-prediction column moves with implementation work, but if the sa...
4. The 1,957-module tied-weight sweep has 371 Verified and 1,577 Abstain, with the runtime false-Verified subset N=6 (the round-3 hand-built positives). The Verified-but-not-runtime-checked count is 365, and the appendix declares this a "sc...
5. The `ShapeGuardModule` Theorem-5 fixture is a single curated module of the kind the prior reviewer asked for, but it is N=1. It demonstrates that the falsification predicate is *capable* of firing on a non-INT event, which is the right q...
6. The 488-block unbind-rerun appendix reclassifies 199/488 corpus rows as "Abstain / not-analysable" because the block-extractor strips the surrounding class context and the analyser then emits `[MODEL_CHECK] No nn.Module subclass found in...
