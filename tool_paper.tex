...
conditionals with path-sensitive joins: both branches must execute from the same
incoming environment and agree on the joined output environment before
downstream code can run.  The abstention theorem is intentionally non-vacuous:
any branch point that produces a safe environment is characterized as a
supported, successfully joined conditional, so unsupported tensor-value branch
conditions cannot silently certify a module.  Conformance tests connect that
abstract theorem to the shipped API by showing \texttt{self.training} branches
are not static fragment violations, while a tensor-value branch whose two real
PyTorch paths both have shape \((2,8)\) still yields \unknown{} in sound mode.
The registry surface is synchronized with the current 136 non-heuristic
operator-confidence entries, while complex operators still rely on their local
conformance tests rather than being over-claimed in the composition proof.
