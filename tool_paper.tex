...
watch mode, Jupyter, decorators, framework adapters, deployment gates, ONNX
shape-inference round trips, model-checkpoint schema gates,
optimizer-state resume gates, CUDA graph capture diagnostics, and artifact scripts that regenerate
precision/recall, latency, ablation, coverage, conformance, fuzzing, mutation,
and benchmark evidence.  This paper describes
...
The numeric-claim audit checks the maintained paper and README against committed
artifacts before release.  The audited headline set includes Refuted-Proof on
53/60 historical bugs, 88.3\% in the headline regime, raw refute count = 56/60,
the fragment-fair comparison 32/34 for TensorGuard versus 25/34 for PyTea with
p=0.0156, 9/9 HuggingFace natural shape bugs, 0/488 unconditional Refuted-Proof
in the unrestricted block corpus, 26/356 in the empty-assumption subset, and $5$ fire inside the audited handler footprint.  The README-facing audit also pins
16/16 frozen benchmark verdict matches, 6/8 buggy eager-runtime failures, 0\%
false positives in high-confidence mode, and the environment-qualified Lean
operator-rule audit (28 rules).
...
annotations, coverage artifacts, dashboards, and certificates.  In certificate
mode the public API now returns the assembled \texttt{SafetyCertificate}, the
merged proof-certificate DAG, and a replay status computed after
\texttt{verify\_architecture} has produced its top-level result, so certificate
validity is visible at the same boundary as the user-facing verdict.  Safe
certificates can also be emitted as deterministic, HMAC-SHA256 signed JSON
artifacts whose payload binds source, analysis configuration, dependency, and
soundness-contract fingerprints.  CI can authenticate those artifacts and replay
the embedded proof DAG with either the public replay helper or a standalone
standard-library checker, without re-running Z3.

Adoption is deliberately copy-pasteable.  The \texttt{tensorguard
adoption-recipes} command emits tested one-line setup commands for GitHub
Actions, pre-commit, pytest, nox, tox, Makefile, VS Code, JetBrains, Neovim, and
Jupyter workflows.  Each recipe is pinned to a committed example repository and
to the actual integration files it configures, so the install surface is tested
against real code rather than documented from memory.  The same philosophy now
extends to a twenty-five-case model gallery: each row pairs a pasteable clean
\texttt{nn.Module}, a minimally changed buggy variant, the exact
\texttt{tensorguard verify} input-shape command, and a test that executes the
clean model under eager PyTorch while proving the buggy source is refuted by the
verifier.  The gallery is now backed by a public model-zoo certification queue:
each job has a content-addressed identifier, deterministic status badge,
expected/observed clean and buggy verdicts, and a failure explanation if monthly
freshness checks observe drift.  The tutorial surface is generated the same way:
ten Colab-runnable notebooks are grouped into seven tracks---shapes, attention, export,
distributed, quantization, community stubs, and formal certificates---and the
CI smoke test executes the committed code cells rather than merely checking that
the notebooks render.  The public documentation site follows the same
repository-backed rule: \texttt{reproducibility/docs\_site.py} emits static
GitHub Pages under \texttt{docs/site/} with concept chapters, an operator
reference, proof-footprint pages, and migration guides, all sourced from the live
soundness contract, verifiable-fragment tables, operator-confidence registry,
proof-footprint manifest, and committed benchmark artifacts.
Contributor onboarding is generated from those same registries: the repository
publishes copyable good-first operator issues, a pytest transfer template, a
declarative stub-authoring guide, and Lean proof examples whose theorem names are
checked against the committed proof tree.
The public launch surface is generated in the same style: demos, evidence
ledgers, upstream RFCs, and compatibility/support promises are tied together by
an auditable campaign rather than a one-off announcement.  The launch path is
also dry-run in a fresh temporary workspace: the harness creates an isolated
virtualenv, installs the checkout without network dependency resolution, runs
the public quickstart through the real \texttt{tensorguard} console script, then
materializes a generated model-gallery bug and proves the same fresh install
reports \unsafe{} before the matching eager PyTorch kernel would fail.  The
shareable terminal capture is generated from that dry-run JSON into
\texttt{docs/launch/quickstart\_terminal\_demo.gif}, so the GIF cannot drift from
the committed fresh-machine evidence.  A companion editor capture,
\texttt{docs/launch/editor\_lsp\_demo.gif}, is generated from a real
\texttt{verify\_architecture} result rendered through the shared LSP provider,
so the squiggle, hover, and quick-fix it shows are backed by the same payload an
editor client receives.

The editor surface is likewise centralized.  A real JSON-RPC language server
over stdio analyzes unsaved buffers and publishes diagnostics, hover shapes, and
quick-fix code actions from the shared \texttt{lsp\_provider}.  VS Code,
Neovim, and the JetBrains plugin template launch that same
\texttt{src.lsp\_server} process, keeping editor support as transport glue
rather than a second verifier; the generated editor GIF is a compact visual
proof of that boundary.

Extension is split into two trust levels.  Untrusted community contributions use
declarative JSON stubs with mandatory provenance and executed conformance cases;
they cannot carry Python code.  Trusted library authors can instead use the
stable operator-plugin ABI in \texttt{src.operator\_plugin\_abi}: a versioned
\texttt{OperatorTheoryContract} supplies an executable shape transfer, provenance,
human security-review attestations, and conformance examples.  TensorGuard never
auto-discovers or auto-imports such plugins; an integrator must explicitly import
the package, validate the contracts, and install only the passing transfers.  A
focused regression test installs a third-party last-dimension theory, verifies a
real \texttt{nn.Module} through it, rejects future-major ABI versions, failed
security attestations, conformance drift, and crashing transfers, and confirms the
no-autoload policy is documented.  The newer
\texttt{src.third\_party\_conformance} layer turns this into a library-facing
certification suite: authors can install their stub or plugin in an isolated
registry snapshot, run real \texttt{verify\_architecture} scenarios, and assert
the expected \safe{}/\unsafe{}/\unknown{} outcome under \texttt{sound},
\texttt{balanced}, and \texttt{heuristic} modes before claiming TensorGuard
compatibility.  The same acceptance story is now generated as maintainer-facing
governance: third-party SMT backends, stubs/plugins, corpora, and signed
benchmark submissions each carry automated gates plus explicitly human
security/governance review items, with every cited script, workflow, and code
symbol checked by a deterministic audit.
...
GGUF/llama.cpp-oriented checks, model checkpoint load preflights,
optimizer-state load/resume preflights,
CUDA graph capture eligibility diagnostics, source-mapped Dynamo/export
graph-break attribution, framework hooks, and deployment
request/response schemas.  The ONNX path validates structural checker and opset contracts, then
round-trips output shapes predicted from captured \texttt{torch.export} metadata
against \texttt{onnx.shape\_inference} wherever both sides infer concrete
dimensions.  These paths are important because many production tensor bugs
appear only when a model is compiled, exported, quantized, sharded, or served.
...
for the predicate-bound argument, plus a versioned serialized predicate-record
replay proof: append-only CEGAR histories are monotone at the actual Python
record boundary and infeasible replay abstains instead of reporting \safe{}.
...
The trust label is now itself auditable.  The generated
\texttt{proof\_footprint\_manifest.json} maps every one of the 139 live
registered transfer functions to both a confidence tag and an evidence tier:
explicit Lean theorem, pen-and-paper rule, tested-only rule, or heuristic.  The
public \texttt{operator\_confidence\_table.json} is a projection of that manifest,
not a parallel hand-maintained classifier.  In the current artifact the manifest
records 16 Lean-backed operator rows, 102 pen-and-paper static rules, 18
tested-only library contracts, and 3 honest heuristics.  Lean rows name the
imported module and theorem declaration; broad families are deliberately not
promoted to Lean status unless that per-operator evidence exists.
...
convolution families, pooling, flatten, reshape/view with product and
stride-viewability reasoning,
...
jagged pooled embeddings, Lean-backed recurrent state contracts,
Lean-backed sparse COO/CSR/CSC/BSR/BSC layout invariants,
...
refinement/alignment, vmap/functorch batch dimensions, \texttt{torch.func} autodiff
wrappers (\texttt{grad}, \texttt{jacrev}, \texttt{jacfwd}, \texttt{jvp}, and
\texttt{vjp}), distribution batch/event/log-prob shapes including
\texttt{TransformedDistribution} identity/reshape and real transform-shape
oracles, grid sampling, affine grids, source-level
...
\texttt{KLDivLoss}, exact machine-checked RNN/LSTM/GRU output and returned-state contracts,
LoRA adapter compatibility, training-loop checks, quantization/export safety,
mixed-precision/autocast dtype-divergence gates, FSDP/FSDP2/DTensor sharding,
explicit pipeline-stage boundary contracts, tensor-parallel sharding checks,
model \texttt{state\_dict} schema compatibility, optimizer-state checkpoint
compatibility, CUDA graph capture eligibility, and TorchServe/FastAPI serving schemas.
...
Sparse tensors are checked beyond construction and now carry a Lean-backed
constructor contract.  \tg{} verifies COO index/value tails; CSR/CSC batch,
\texttt{nnz}, compressed-axis, and dense-tail invariants; BSR/BSC blocksize and
divisibility invariants; sparse-dense
...
successful output to live PyTorch via dense shape equality, use
\texttt{check\_invariants=True} where compressed constructors otherwise validate
lazily, and keep unsupported BSR/BSC sparse matrix kernels as explicit
non-passes rather than build-dependent silent approvals.
...
heuristic.
The corresponding Lean artifact proves layout preservation, \texttt{batch\_first}
state-axis selection, bidirectional output/depth doubling, LSTM-only cell-state
availability, and the projected bidirectional LSTM asymmetry checked by the
...
embedding compatibility, ordinary leading batch/head broadcast, attention-mask
broadcast against \((\ldots,L_q,L_k)\), and the scoped
\texttt{enable\_gqa=True} caveat: key/value heads divide query heads on the
\(-3\) axis and the output uses query heads plus the value embedding dimension.
Multi-head attention checks packed and unpacked query/key/value forms,
\texttt{kdim}/\texttt{vdim}, batch-first layout, attention masks, key-padding
masks, per-head weight shapes, and valid symbolic cases.  Tests replay these
contracts against real PyTorch modules and functional calls.
...
the inference chain from inputs through transformations to the failing step, and
\texttt{tensorguard explain} turns that evidence into a self-contained HTML
report with an SVG chain graph, the concrete counterexample witness,
proof-footprint badges for the operators in the chain, and any mechanical repair
...
Adoption surfaces include a GitHub Action, SARIF for code scanning, a
SARIF-to-Code-Scanning trend dashboard that tracks open, closed, recurrent, and
per-release alert deltas, JSON and JUnit reporters, GitHub PR annotations,
pre-commit integration, nox/tox hooks, a pytest plugin, a watch mode,
Jupyter/IPython cell checks, a local static playground generated by
\texttt{tensorguard playground} (editable examples, precomputed verdict
snapshots, no upload/import/execution), a
...
model-hub release path can also emit a \texttt{TensorGuard-verified} badge:
\texttt{tensorguard model-hub-badge} verifies the real \texttt{nn.Module} in
sound mode, signs the replayed safety certificate, writes a manifest plus SVG badge,
and produces a copy-paste model-card snippet anchored by source and payload
SHA-256 hashes.  For teams that want adoption visibility without phoning home,
\texttt{tensorguard usage-metrics} locally summarizes JSON reports into hashed
file counts, verdict/abstention totals, bug categories, and unsupported-op
hotspots; it reads no source and performs no network I/O.  The
...
When \texttt{torch.compile}/Dynamo or \texttt{torch.export} cannot capture a
model, \tg{} now preserves the failure as a structured graph-break attribution
instead of an opaque backend exception.  The report maps the failing source site
back to the verifiable-fragment taxonomy---for example data-dependent Python
branching, tensor-to-scalar extraction, dynamic assertions, data-dependent
iteration, opaque helper calls, or mutation/JIT/custom-autograd boundaries---and
attaches the exact snippet, confidence, reason, and smallest mechanical rewrite
the user should try, such as replacing tensor-value branching with
\texttt{torch.cond}/\texttt{torch.where} or moving scalar extraction outside
\texttt{forward}.  Dynamo-to-FX fallback keeps this metadata, so successful
fallback verification does not erase the explanation of why the stronger backend
failed.
...
\texttt{experiments\_v5/github\_bug\_mining/} records 2,704 public issue and PR
signals associated with PyTorch runtime error signatures; its provenance-rich
expansion adds explicit license metadata, direct or candidate PR commit-page
links when available offline, and repo-authored minimized reproducers for all
eight runtime-signature categories.  A separate dual-pass labeling agreement
audit samples those mined rows, publishes the rubric, ambiguity taxonomy, and
adjudication log, and reports kappa for inclusion/root-cause/evidence judgments
in \texttt{experiments\_v5/labeling\_agreement/agreement.json}; it is explicitly
not a human-subjects study and does not treat the mechanical category label as
the headline agreement measure.  Fuzzing harnesses generate valid modules
and mutation harnesses inject shape, device, and dtype faults.  Differential
conformance tests cross-check transfer functions against the live torch
dispatcher.  Baseline comparison scripts evaluate PyTea and runtime-tool
alternatives.  Dashboards and JSON artifacts track coverage, precision/recall,
latency, ablations, false positives, and statistical rigor.

\texttt{reproducibility/head\_to\_head\_step252.py} broadens the same-case
baseline comparison to PyTea, Pyright, jaxtyping/torchtyping availability,
\texttt{torch.export}/Dynamo guard paths, runtime smoke tests, and a frozen
GPT-4.1-nano bug-finder run.  On the twenty shared cases, \tg{} is 10/10 on
bugs and 10/10 on clean programs; runtime smoke tests miss the two branch-latent
Optional bugs under the fixed single-path input, Pyright catches exactly those
Optional bugs but not tensor-shape mismatches, and the LLM artifact is reused
only after hash and label alignment checks, avoiding any fresh code upload.

A new protocol registry, \texttt{reproducibility/evaluation\_protocol.py},
turns the methodology itself into a checkable artifact.  It does not score
\tg{}; it hashes the development corpus, primary held-out blind split, external
real benchmark, and every governed analysis script; proves the dev/blind case
IDs are disjoint; freezes the no-blind-tuning and negative-result policies; and
records the exact formulas for recall, specificity, false-positive rate,
precision, abstention, Wilson intervals, paired McNemar tests, effect sizes,
power, and overfitting gap.  The one-command reproduction pipeline regenerates
this protocol before the downstream scoring scripts it governs, while the paper
evidence index and dashboard expose the protocol hash alongside the results.
...
\hline
Evaluation protocol &
\texttt{reproducibility/evaluation\_protocol.py},
\texttt{tests/test\_evaluation\_protocol.py}.  Pre-specified split registry,
tuning freeze, metric definitions, script hashes, and reproduction-order guard
before governed scoring artifacts. \\
\hline
Ground-truth corpus &
...
\hline
Provenance-rich bug corpus &
\texttt{experiments\_v5/provenance\_bug\_corpus/},
\texttt{tests/test\_provenance\_bug\_corpus.py}.  Deterministic 2,704-record
expansion with license status, runtime signatures, PR/candidate commit-link
coverage, and legal category-level reproducers checked against real PyTorch. \\
\hline
Labeling agreement &
\texttt{experiments\_v5/labeling\_agreement/},
\texttt{tests/test\_labeling\_agreement.py}.  Dual-pass metadata audit over a
deterministic 32-record stratified sample, with rubric, ambiguity taxonomy,
adjudication log, and regenerated kappa/confusion metrics for inclusion,
root-cause, and evidence-strength judgments. \\
\hline
Controlled developer-study packet &
\texttt{reproducibility/developer\_study.py},
\texttt{tests/test\_developer\_study.py}.  Registered localization-time,
fix-quality, and trust-calibration task packet over real marker-bearing bugs;
it includes participant materials and scoring keys while explicitly claiming no
executed human-subjects outcomes. \\
\hline
PR-history survival &
\texttt{reproducibility/pr\_history\_survival.py},
\texttt{tests/test\_pr\_history\_survival.py}.  Offline fix-linked study over
332 GitHub PR/issue records: every direct or candidate-linked record maps to a
runtime-signature witness that current \tg{} rejects before execution, with
category-level scope made explicit and structural CI-cost proxies reported. \\
\hline
Tutorial tracks &
\texttt{examples/tutorials/},
\texttt{tests/test\_tutorial\_notebooks.py}.  Generator-backed Colab notebooks
for shapes, attention, export, distributed, quantization, governed stubs, and
formal certificates; the focused test executes the code cells and checks the
track manifest so onboarding examples cannot silently drift. \\
\hline
Precision/recall &
\texttt{tests/test\_precision\_recall.py},
\texttt{tests/test\_hard\_recall.py},
\texttt{tests/test\_false\_unknowns.py}, \texttt{experiments/}.  Tracks caught
bugs, clean-model behavior, high-confidence safety, and whether strict sound
mode over-abstains on executable ground-truthed cases. \\
\hline
...
and injected bugs; the differential fuzzer now checks the same eval phase it
executes, preserving the 200/200 clean-model zero-false-positive invariant even
for BatchNorm edge cases with one value per channel. \\
\hline
Runtime-silent bugs &
\texttt{reproducibility/silent\_bug\_benchmark.py},
\texttt{tests/test\_silent\_bug\_benchmark.py}.  Curated CPU-only benchmark of
15 non-raising semantic failures across gradient freezes, stale buffers,
optimizer-state drift, train/eval mode leakage, and quantization parameters,
with independent semantic oracles and object-level gates. \\
\hline
Operator coverage &
\texttt{operator\_confidence\_table.json},
\texttt{proof\_footprint\_manifest.json},
...
operators, manifest-derived confidence tags, evidence tiers, release gates, and
frequency-weighted real-model coverage over torchvision, timm, and HuggingFace
traces. \\
\hline
Latency and scaling &
\texttt{tests/test\_cost\_benchmarks.py},
\texttt{tests/test\_latency\_budgets.py},
\texttt{tests/test\_scaling\_study.py},
\texttt{tests/test\_pareto\_curves.py}.  Measures model-size, operator-coverage,
solver-budget, abstention, and hardware-normalized latency trade-offs. \\
\hline
Baselines &
\texttt{tests/test\_baselines.py},
\texttt{tests/test\_baseline\_head\_to\_head.py},
\texttt{tests/test\_headtohead\_n15.py},
\texttt{tests/test\_head\_to\_head\_step252.py}.  Compares against PyTea,
Pyright, runtime smoke tests, export/Dynamo guards, type-oriented alternatives,
and a frozen LLM baseline where applicable. \\
\hline
Ablations &
...
\texttt{tests/test\_stagewise\_ablation.py},
\texttt{tests/test\_verified\_reduction\_diff.py}.  Measures contribution of
domains, cross-domain reductions, stubs, proof policy, and refinement depth. \\
\hline
Dashboards &
\texttt{tests/test\_dashboard.py}, \texttt{tests/test\_build\_dashboard.py},
\texttt{tests/test\_leaderboard.py},
\texttt{tests/test\_leaderboard\_submission.py},
\texttt{tests/test\_model\_zoo\_certification.py},
\texttt{tests/test\_paper\_results\_freshness.py}.  Regenerates static result
views for reviewers and contributors, including the signed public leaderboard,
monthly freshness cadence, SSH trust anchor, anti-overfitting intake rules,
status-badged gallery certification jobs, and a paper-results freshness bot that
reruns artifact checks before applying benchmark-delta gates. \\
\hline
Statistical rigor &
\texttt{tests/test\_statistical\_power.py},
\texttt{tests/test\_significance.py},
\texttt{tests/test\_corrected\_meta\_analysis.py},
\texttt{tests/test\_statistical\_meta\_analysis.py}.  Documents effect sizes,
power, and cross-corpus evidence without naive synthetic/real pooling. \\
\hline
\end{longtable}
...
ONNX shape-inference disagreements, mixed-precision/autocast dtype drift,
FSDP2/DTensor local-shard mistakes, and tensor-parallel sharding mistakes.
Checkpoint-schema gates add another resume-only failure class: model
\texttt{state\_dict} loads are checked for missing or unexpected keys, tied
weights whose aliases disagree, adapter-only LoRA matrices, tensor-parallel
shard coverage, and dtype drift that PyTorch would otherwise silently cast.
The dedicated LoRA/PEFT gate checks HuggingFace-style adapter keys and live
modules for rank, target-module matching, merged/unmerged state, and
quantized-base compatibility.  Optimizer-state checks cover AdamW and fused-AdamW moment
buffers, Adafactor factored \texttt{row\_var}/\texttt{col\_var} buffers, lazy
pre-step state, and declared ZeRO-style shards before an incompatible checkpoint
can corrupt a resumed run.
The silent-bug benchmark extends this beyond exceptions: 15/15 curated CPU cases
execute without raising yet are oracle-positive for gradient freezes, stale
buffers, optimizer-state drift, train/eval mode leakage, or quantization output
drift; the corresponding object-level gates catch all 15 while the artifact
explicitly labels the corpus as coverage/regression evidence rather than a field
prevalence estimate.

The statistical synthesis follows the same honesty rule.  The Step-265
meta-analysis consumes the extended real-bug corpus, hard-recall latent bugs,
runtime-silent semantic bugs, mutation tests, negative and differential fuzzing,
false-\unknown{} checks, clean stress tests, natural clean samples, and same-case
baseline artifacts, but it does not publish one giant pooled denominator.  Each
artifact is first reduced to a suite-level Bernoulli estimate tagged by
distribution; the primary interval is a deterministic cluster bootstrap within
that distribution.  Case-weighted rates and random-effects logit summaries are
reported only as diagnostics, so a large generated suite cannot wash out a small
real-bug suite.

The same machinery includes explicit negative controls.  Six value-dependent
programs produce NaNs, infinities, or failed runtime value assertions on crafted
inputs while remaining structurally well-formed.  \tg{} catches 0/6, as it
should: arbitrary value semantics are outside its declared tensor-structure
contract.  A plain eager smoke test catches only the assertion cases, while an
explicit finite-output runtime checker catches 6/6.  Reporting that loss keeps
the claim sharp: \tg{} is a static tensor-contract verifier, not a replacement
for value-domain tests.
...
The newest stage-wise ablation ties these local claims into one live stack:
source extraction builds a real graph and catches a seeded mismatch; every
verification domain is load-bearing while phase is diagnostic-only; every
registered reduction rule has a concrete refining witness, including the
truthiness-triggered rule outside the default engine; CEGAR reaches full
contract-level diagnostics at depth one without false alarms; a third-party stub
turns an opaque clean model into a safe analyzable one while still rejecting bad
input contracts; and strict sound mode accepts the Lean-backed \texttt{torch.relu}
case while abstaining on heuristic \texttt{torch.unique} with an explicit reason.

The PR-history survival study asks a complementary operational question: if a
TensorGuard gate had run before a fix-linked GitHub PR merged, would the
runtime-signature category have been rejected?  Because the provenance corpus
does not redistribute historical pre-fix source or patches, the answer is
deliberately scoped to category-level replay: 332 fix-linked issue/PR records
provide the real GitHub multiplicities, while the eight runtime-signature
families are replayed through trusted source witnesses and the repo-authored
minimized PyTorch reproducers.  The result is a fully deterministic before-merge
survival estimate with zero historical-checkout overclaim: direct PR rows,
candidate same-repository issue links, device-CUDA-qualified evidence, dtype
runtime-family caveats, detection depth, and CI cost are all explicit fields in
\texttt{reproducibility/pr\_history\_survival.json}.

The clean natural-distribution study checks the other side of deployment risk:
whether ordinary public-repository-style models trigger annoying alarms or
abstentions.  The committed Step-258 harness scores 174 clean model instances
across fourteen architecture families and nine provenance strata, including
torchvision/timm-style CNN and normalization motifs, HuggingFace embedding and
sequence motifs, nanoGPT-style transformer blocks, U-Net upsampling motifs, and
PyTorch example-style MLP, recurrent, autoencoder, Siamese, and generative
models.  Each compact source is a redistributable motif reimplementation rather
than vendored third-party code, and each executes under eager PyTorch with its
declared inputs before it is counted.  In all three soundness modes \tg{}
returns a decided \safe{} verdict on every case: 174/174 coverage, zero
abstentions, zero false alarms, and a Wilson 95\% upper bound of 2.16\% on the
false-positive rate for this clean natural sample.  The artifact also records
abstention-cause buckets; in this run all buckets are empty, so the result is
not hiding an opaque long tail behind a mean.

The false-\unknown{} benchmark makes that abstention claim sharper for the
strict soundness boundary.  \texttt{evaluation/false\_unknowns.py} runs
\texttt{sound} mode on 86 executable, ground-truthed cases that \tg{} should
decide: eight frozen real clean benchmarks, seventy-two generated clean models
admitted only after eager PyTorch execution, and six phase/path latent bugs whose
hidden faults are confirmed by the hard-recall validators.  The result is
86/86 decided, zero false \unknown{} verdicts, and zero misclassifications, with
counts broken down by kind and family so any future abstention regression has an
immediate prioritization table.
...
CEGAR convergence bounds, infeasible-contract abstention tied to the versioned
Python predicate serialization format, fragment modes, device/dtype transfer
soundness, SMT encoding faithfulness, cross-domain
composition, and operator-specific rules such as cat, chunk/split partition
reconstruction, reshape, contiguity/stride/storage-offset viewability with
channels-last caveats, einops decomposition/bijection families, SDPA
broadcast/mask/GQA shape laws, RNN/GRU/LSTM state-shape laws,
distribution batch/event/log-prob algebra,
TransformedDistribution identity/reshape event-rank rules, and named-tensor
refine/align name preservation, singleton insertion, and duplicate-name
rejection.  The sparse-layout artifact proves accepted COO/CSR/CSC/BSR/BSC
constructor models materialize to exactly the requested dense shape, with
theorem-shaped rejection witnesses for bad compressed lengths, block
divisibility failures, and unusable dense tails.
The SMT bridge is now explicit for the solver constraints that recur across the
implementation: \texttt{SmtEncoding} proves that broadcast compatibility
formulas, reshape divisibility formulas, split/chunk section-sum formulas, and
finite dtype-promotion table/chain formulas are satisfiable exactly when the
corresponding Lean transfer guard accepts.  The companion parity tests instantiate
those theorem shapes as real Z3 formulas and compare them against \tg{} helpers
and live PyTorch behavior.
The Lean audit checks theorem statements for sorry-free status under the
trusted kernel axioms used by the environment.  The proof-footprint manifest
then gives users a per-operator index into this trust story: \texttt{matmul},
\texttt{mm}, \texttt{bmm}, \texttt{gather}, \texttt{scatter},
\texttt{stack}, \texttt{squeeze}/\texttt{unsqueeze}, selected reductions, and
broadcast helpers point at concrete Lean declarations, while linalg and FFT
contracts remain correctly labeled as tested-only until they receive their own
per-operator proofs.

The formal appendix is now generated from the same sources rather than copied
from prose.  \texttt{formal\_soundness\_appendix.tex} mines namespace-qualified
Lean theorem statements, resolves every Lean-backed proof-footprint row to its
source declaration, and embeds the verifiable-fragment grammar and supported
construct tables.  This makes the paper's trust boundary auditable as a
repository artifact: if a theorem, operator tier, or fragment production drifts,
the appendix regeneration test fails.

The newest formal layer lifts those local facts to whole modules.
\texttt{TensorGuard.SubjectReduction} defines a small environment-based tensor
language whose statements append outputs from primitive proof-backed families
or from explicit proof-carrying local transfer certificates.  It proves
\texttt{step\_subject\_reduction}, \texttt{exec\_subject\_reduction}, and
\texttt{whole\_module\_subject\_reduction}: if every local complete/sound
transfer succeeds, every intermediate and final tensor in the straight-line
fragment remains well-formed.  The same artifact now models supported
conditionals with path-sensitive joins and statically bounded loops:
ModuleList/Sequential and literal-range loops are represented as explicit
unrolled body lists, and a successful bounded-loop proof implies that the
resolved body count is within the configured unroll limit.  The abstention
theorems are intentionally non-vacuous: any branch point that produces a safe
environment is characterized as a supported, successfully joined conditional,
and over-budget or unsupported loops evaluate to no safe environment.  The
shipped extractor mirrors that proof boundary by recording loop abstentions and
turning them into \unknown{} in sound mode.  Conformance tests connect the
abstract theorem to the API: \texttt{self.training} branches remain supported,
joinable tensor-value branches still abstain, under-limit ModuleLists verify
against real PyTorch, and over-limit ModuleLists are tolerated only outside the
strict sound contract.  The registry surface is synchronized with the current
136 non-heuristic operator-confidence entries, while complex operators still
rely on their local conformance tests rather than being over-claimed in the
composition proof.
...
axes, preventing false claims about a final chunk size.  The
\texttt{ChunkSplit} Lean artifact proves the axis reconstruction law used when
all produced pieces are concatenated back together, while generated conformance
tests compare uneven chunks, zero sections, and fewer-than-requested chunks
against live PyTorch.  The named-tensor Lean artifact follows the same trust
split: Lean proves the no-ellipsis core for preserving existing names, inserting
singleton axes for fresh or anonymous targets, rejecting duplicate concrete
names, and rejecting unnamed inputs that need an ellipsis; conformance tests
replay those cases against real \texttt{refine\_names} and \texttt{align\_to}.
...
checking, graph compilation, torch integration, compile-guard comparison, export extraction, Dynamo gap
...
entry points with soundness modes, domain flags, and top-level
\texttt{SafetyCertificate} replay for proof-carrying safe verdicts. \\
\hline
...
compiler or exporter failures, then validate ONNX checker/opset and
shape-inference round trips on emitted artifacts. \\
\hline
...
\texttt{src/gguf\_export.py}, \texttt{src/checkpoint\_verify.py},
\texttt{src/optimizer\_state\_verify.py},
\texttt{src/distributed\_verification.py}, \texttt{src/tensor\_parallel\_checks.py},
\texttt{src/lora\_verification.py}, \texttt{src/cuda\_graph\_capture.py} &
Deployment, mixed-precision, distributed training, and adapter-compatibility
gates, including GGUF/llama.cpp checkpoint preflight checks, model
\texttt{state\_dict} schema checks, AdamW/Adafactor optimizer-state resume
checks, and FSDP2/DTensor
rank-local shard contracts checked against PyTorch's own placement splitter,
explicit pipeline activation-boundary contracts, and MQA/GQA tensor-parallel
attention contracts, plus CUDA graph capture diagnostics for dynamic allocation,
data-dependent shapes, unsupported operations, and static input replay. \\
\hline
...
\hline
\texttt{src.operator\_plugin\_abi.py}, \texttt{src.stub\_governance.py},
\texttt{src.third\_party\_conformance.py} &
Governed extension surfaces: declarative community stubs, trusted executable
operator theories, security/provenance review, transfer conformance cases, and
real verifier-mode certification suites for third-party libraries. \\
\hline
...
indices, including the distribution-stratified meta-analysis that keeps
synthetic, natural, stress, and real-bug evidence separate.  A newer
\texttt{make paper-results-freshness} target is the scheduled paper-results bot:
it reruns the committed freshness checks for precision/recall, fuzzing, hard
recall, triage, and deployment dashboards, then normalizes the paper-facing
dashboards into snapshots and compares them with
\texttt{scripts/benchmark\_delta.py}.  This caught stale
\texttt{confusion\_matrices.json} and \texttt{diff\_fuzz.json} during this
revision, and forced the eval-phase differential-fuzz oracle fix described
above before the paper artifacts could pass.  For artifact review,
\texttt{scripts/reproduce\_main\_results.sh} is a
single reviewer-facing entrypoint to the authoritative byte-checking pipeline,
while \texttt{scripts/check\_main\_results.sh} verifies and previews the command
catalogue.  The generated \texttt{reproducibility/reviewer\_commands.json} and
\texttt{.md} manifest lists 14 main result groups, checks that every command
resolves to a real Makefile target or Python script, and checks that every
claimed output exists.  The companion
\texttt{reproducibility/artifact\_index.json} and \texttt{.md} files form a
tamper-evident ledger for generated outputs: every non-index result, generated
corpus file, and volatile regenerated artifact records byte size and SHA-256,
and an aggregate root digest changes if any indexed path, size, or content hash
changes.  Tests pin committed JSON artifacts against freshly generated outputs.
The benchmark corpus has content hashes, datasheets, citation metadata, and
loader checks; the mined-bug expansion carries a deterministic corpus hash plus
explicit no-third-party-code redistribution metadata; the labeling-agreement
artifact pins its sample seed, annotation hash, taxonomy hash, adjudication log,
and regenerated kappa/confusion tables.  The evaluation protocol
is regenerated as a first-class deterministic artifact, so split roles, tuning
locks, metric formulas, and scoring-script hashes drift only through reviewed
source changes.  The operator confidence table and proof-footprint manifest are
generated from one source of truth and pinned against committed JSON: the
confidence table is mechanically projected from proof-footprint rows.  The dashboard is
regenerated from committed results.  Proof-carrying certificates are replayed
through the public API boundary, not only inside solver helpers.  The public docs
explain what the verifier can and cannot prove.

Version stability is treated as an artifact, not a promise.  The Step-257
matrix \texttt{reproducibility/cross\_version\_stability.py} scores all 227
extended-corpus cases with target \texttt{torch} and \texttt{torchvision} imports
blocked, then re-scores a deterministic sample plus non-vacuous source-level
fixtures for \texttt{torchvision.transforms.v2}, CUDA-less CPU, and MPS under
qualified PyTorch 2.1--2.9 and torchvision 0.16--0.24 version matrices.  It
links the Python 3.9--3.14 hash-seed determinism proof and records Linux/macOS
and backend full-matrix commands as environment-qualified evidence rather than
claiming every wheel and backend was installed on one host.
...
surface--CLI, CI, SARIF, Jupyter, decorators, production trainer adapters,
export gates, Docker, conda, pipx, docs--that adoption can be evaluated rather than imagined.
...
Community comparisons now flow through a signed public leaderboard: submissions
are SSH-authenticated against a maintainer trust anchor, re-scored from raw
verdicts, checked monthly for freshness, and reviewed under explicit
anti-overfitting rules.  Copyable gallery models flow through a parallel
certification queue with deterministic badges and failure explanations, so demos
do not silently drift from the verifier.  The next natural milestones are larger
blind real-bug corpora, broader deployment galleries, and independently
...
subject-reduction and SDPA proof work illustrates the project's standard: match
PyTorch's exact edge-case semantics, compose local certificates through real
module structure, abstain on symbolic or value-dependent uncertainty, and pin
the behavior with Lean plus live-library tests.
...
convolution, pooling, fold/unfold, embedding, and attention rules, including
Lean-checked recurrent state laws, SDPA masks, and GQA heads. \\
\hline
Device reasoning &
...
batch-first layout, grouped head variants, GQA head divisibility,
MQA/GQA tensor-parallel partitioning, KV-head replication, and sequence-parallel
LayerNorm safety. \\
\hline
Embedding bags &
...
\hline
Sparse tensors &
Lean-backed COO/CSR/CSC/BSR/BSC shape and blocksize invariants, sparse-dense
...
\hline
Named tensors &
\texttt{refine\_names} and \texttt{align\_to} alignment checks, with
Lean-backed preservation, singleton-insertion, and duplicate-name laws. \\
\hline
Complex and FFT &
...
dtype, device, dynamic-guard, ONNX opset availability, lowering checks, and
post-export ONNX shape-inference agreement.  The compile-guard audit compares
\tg{} rank, concrete-dimension, repeated-symbol, and layer-boundary constraints
against Dynamo \texttt{ExplainOutput.out\_guards}, reporting missing shape-env
guards rather than trusting a compiled graph by inspection.  GGUF/llama.cpp export gates check
q/k/v packing, rotary dimensions, vocab projections, quant block divisibility,
and metadata consistency before an external converter is invoked; model
checkpoint gates check missing/unexpected keys, tied weights, adapter-only LoRA
matrices, tensor-parallel shards, and silent dtype drift; LoRA/PEFT gates check
rank, target-module matching, merge state, and quantized bases; optimizer-state gates check
AdamW, Adafactor, fused AdamW, and sharded resume buffers before
\texttt{load\_state\_dict}; serving-schema gates check request fields,
preprocessing tensor layouts, model outputs, and response payloads before a
TorchServe/FastAPI wrapper turns a bad batch into an opaque inference failure;
CUDA graph gates check allocation-free captured regions, value-independent
output shapes, unsupported host-synchronizing operations, and replay-stable
input shape/dtype/device/stride contracts before capture. \\
\hline
Certificates and replay &
Top-level \texttt{SafetyCertificate}s bind SAFE verdicts to proof-DAG replay
data; signed artifacts add canonical payload hashes, HMAC-SHA256 authentication,
and source/config/dependency/soundness drift fingerprints.
\texttt{tests/test\_api\_certificate\_replay.py},
\texttt{tests/test\_signed\_certificate.py}, and
\texttt{tests/test\_certificate\_checker.py} replay real
\texttt{verify\_architecture} certificates and cross-check the standalone
checker without solver imports. \\
\hline
Distributed and adapters &
DDP/FSDP/FSDP2/DTensor/per-parameter, explicit pipeline-boundary, and
tensor-parallel checks, plus
optimizer-state, checkpoint-schema, and LoRA/PEFT adapter compatibility gates. \\
\hline
Reports and integrations &
CLI, JSON, JUnit, SARIF, GitHub annotations, GitHub Action, pre-commit, pytest
plugin, nox/tox, watch mode, Jupyter, no-upload local playground, decorators,
and framework hooks. \\
\hline
Formal artifacts &
Lean proof files, axiom audits, serialized CEGAR predicate replay,
conditional/straight-line subject reduction, SMT encoding faithfulness checks,
including broadcast/divisibility/partition/dtype-promotion Z3 parity,
proof-footprint manifest rows for every registered operator, a generated formal
soundness appendix linking Lean declarations to the fragment grammar, generated
conformance bridges, and proof certificate infrastructure. \\
\hline
Reproducibility &
...
\hline
\texttt{tests/test\_subject\_reduction\_loops.py} &
Lean-audited bounded-loop subject reduction plus real ModuleList/static-range
unroll limits, sound-mode \unknown{} fallback, and data-dependent iteration
guards. \\
\hline
\texttt{tests/test\_onnx\_export\_gate.py},
\texttt{tests/test\_onnx\_opset\_gate.py},
\texttt{tests/test\_export\_aot\_gate.py},
\texttt{tests/test\_gguf\_export\_gate.py},
\texttt{tests/test\_checkpoint\_verify.py},
\texttt{tests/test\_optimizer\_state\_verify.py},
\texttt{tests/test\_serving\_schema.py},
\texttt{tests/test\_cuda\_graph\_capture.py},
\texttt{tests/test\_quant\_export\_checks.py},
\texttt{tests/test\_quantization\_verify.py},
\texttt{tests/test\_deployment\_budgets.py},
\texttt{tests/test\_deployment\_gallery.py},
\texttt{tests/test\_deployment\_dashboard.py} &
Deployment-facing gates for export, \texttt{torch.export.Dim} ranges and
relations, ONNX opset-version availability and dynamic-shape support, AOT
package input/layout/dtype/device/lowering contracts, and source/eager/FX
quantization, plus GGUF/llama.cpp checkpoint checks over real transformer
state\_dict tensors, logical quantized descriptors, model checkpoint-schema
compatibility for tied weights, LoRA adapters, tensor-parallel shards, and
dtype drift, plus real optimizer state\_dict compatibility for AdamW, Adafactor,
fused AdamW, lazy state, sharded resume buffers, and load-time shape rejection;
FastAPI/TorchServe-style serving pipelines whose request, preprocessing-output,
model-output, and response contracts are checked against real PyTorch modules,
including an NHWC-vs-NCHW preprocessing bug rejected before \texttt{forward},
and CUDA graph eligibility checks that run real modules while flagging dynamic
allocation, data-dependent shape operators, unsupported host synchronization,
and replay input shape/stride drift. \\
\hline
\texttt{tests/test\_distributed\_verification.py},
\texttt{tests/test\_tensor\_parallel\_checks.py} &
FSDP, DeepSpeed, FSDP2, DTensor, per-parameter, pipeline-boundary, and
tensor-parallel shape/device contracts, including rank-specific shard-shape
conformance against PyTorch's \texttt{torch.distributed.tensor} placement helper
and MQA/GQA attention conformance against real PyTorch sharded executions plus
installed HuggingFace \texttt{LlamaAttention} projection shapes. \\
\hline
...
\hline
\texttt{tests/test\_cross\_version\_stability.py} &
Version-qualified stability across Python, PyTorch, torchvision, CUDA-less CPU,
MPS, and Linux/macOS, with blocked-import source-only proofs and deterministic
artifact regeneration. \\
\hline
\texttt{tests/test\_artifact\_index.py},
\texttt{tests/test\_paper\_evidence\_index.py},
\texttt{tests/test\_paper\_capsule\_wiring.py},
\texttt{tests/test\_paper\_results\_freshness.py} &
Paper/artifact wiring so claims can be tied to scripts, committed evidence, and
tamper-evident SHA-256 generated-output hashes; the freshness bot additionally
proves paper dashboards are regenerated and benchmark-delta gated. \\
\hline
\end{longtable}
...
sequences. &
Lean axiom audits, focused recurrent tests, live PyTorch projected bidirectional
LSTM/GRU/RNN shape oracles, and packed-sequence abstention regression. \\
\hline
...
aliases. &
Precise reshape tests, divisibility optimization tests, Lean proofs, and real
model regressions. \\
\hline
Einops &
\texttt{rearrange}/\texttt{reduce}/\texttt{repeat} parse grouped axes,
ellipsis, anonymous constants, reductions, and repeats; grouped-axis
decomposition checks divisibility and rearrange preserves a named-axis
bijection. &
Generated Lean-conformance cases cross-check TensorGuard, real \texttt{einops},
and source-level bug reports for non-divisible decompositions, missing axes, new
axes, and duplicates. \\
\hline
Flatten/unflatten &
...
Lean \texttt{ChunkSplit} reconstruction theorems, generated live-PyTorch
conformance in \texttt{tests/test\_chunksplit\_lean\_conformance.py}, and
downstream Linear/cat model checks. \\
\hline
Squeeze/unsqueeze &
...
\hline
Attention &
Q/K/V ranks, embedding dimensions, source lengths, batch layout, masks,
head-related contracts, MQA/GQA tensor-parallel head partitioning, KV-head
replication, and sequence-parallel LayerNorm axes are checked. &
MHA and SDPA tests against real PyTorch modules and functional calls, including
Lean-conformance SDPA broadcast/mask/GQA cases, plus
tensor-parallel attention tests over real PyTorch sharded executions and
installed HuggingFace \texttt{LlamaAttention} projection shapes. \\
\hline
Normalization &
...
blocksize constraints; accepted layouts prove dense-materialization shape
preservation. &
Sparse verification tests across COO, CSR, CSC, BSR, and BSC plus Lean axiom
audits. \\
\hline
Complex and FFT &
...
operations. &
Named tensor verification tests plus Lean conformance for preservation,
singleton insertion, and duplicate-name rejection. \\
\hline
Distribution shapes &
...
\hline
Export gates &
Export, ONNX, AOTInductor, GGUF, CUDA graph, serving, quantization, and mixed-precision paths check static
...
symbols before tracing; ONNX exports check exporter opset bounds, lowered ATen
minimum opsets, and backend-specific dynamic-shape support before writing a
proto; and AOT packages additionally reject invalid example layouts, devices,
dtypes, missing dynamic guards, and denied ATen lowerings before the packager
writes an artifact.  Compile-guard comparison normalizes Dynamo rank, shape-index,
range, and cross-input equality guards and checks them against \tg{} symbolic
constraints on real \texttt{nn.Linear}, residual, and convolution modules.  The GGUF gate checks architecture-prefixed llama.cpp
metadata, q/k/v and packed-qkv tensor shapes, grouped-query head consistency,
rotary dimensions, vocab/output projection padding, and GGUF quant block sizes
before the converter writes a checkpoint; the optimizer-state gate checks
AdamW/fused-AdamW moment shape, Adafactor factored variance buffers, lazy
initialization, sharded-state coverage, and checkpoint resume compatibility
before \texttt{optimizer.load\_state\_dict}; serving gates validate captured
request fields, preprocessed tensor shapes/dtypes/devices, model-output
contracts, and postprocessed response payloads before invoking or returning from
FastAPI/TorchServe-style wrappers; the CUDA graph gate flags fresh allocation,
data-dependent extents, unsupported capture-time host operations, and replay
shape/dtype/device/stride drift. &
ONNX, AOT, GGUF, CUDA graph, serving-schema, quantization, mixed-precision, compile-guard, and guarded compile/export
tests. \\
\hline
Distributed and adapter gates &
Sharding, explicit pipeline stage boundaries, tensor-parallel linears and
MQA/GQA attention, optimizer state, checkpoints, and LoRA adapters have
shape/dtype/rank contracts layered over base verification; PEFT-style
\texttt{target\_modules}, merged/unmerged flags, and quantized bases are checked
without mutating the model. &
Distributed verification, tensor-parallel, optimizer-state, training-loop,
checkpoint, and LoRA tests, including real PyTorch optimizer state dicts,
real PyTorch sharded attention, and HuggingFace
\texttt{LlamaAttention} projection checks. \\
\hline
\end{longtable}
...
failures.  The Step-249 expansion keeps those records as facts and URLs rather
than redistributing third-party code: long-tail licenses are marked
\texttt{NOASSERTION} instead of guessed, known high-frequency repositories carry
SPDX snapshots, PR rows carry direct commit-page links, issue rows carry
same-repository candidate links when present, and each runtime-signature family
has a minimized TensorGuard-authored reproducer.  The intent is not to treat
every mined candidate as a theorem; it is to create a large, auditable pool that
can be stratified, manually labeled, filtered into blind splits, and reused by
future tools.

The Step-255 agreement artifact adds the missing audit layer for that pool.  It
does not ask reviewers to trust a fabricated human study: the committed
\texttt{annotations.jsonl} is labeled as a dual-pass repository metadata audit,
the rubric states that the mechanical runtime-signature category is not the
headline agreement object, and the generated \texttt{agreement.json} reports
moderate inclusion-decision kappa, stronger root-cause kappa, and high
evidence-strength agreement together with the adjudicated ambiguous rows.

The survival study is the first analysis to consume those fix links directly.
It reports a category-level answer to ``would this have been caught before
merge?'' without claiming to replay private or unredistributed source: linked
GitHub rows count the opportunity, runtime-signature witnesses define the static
check, live PyTorch reproducers validate the failure family, and structural
operation counts estimate the CI work avoided by static detection.

\subsection{Stratified precision and recall}

The precision/recall artifact is now disaggregated before it is quoted.  A
deterministic post-processor consumes the frozen per-model predictions and
reports TP/FP/TN/FN by operator family, framework, bug class, model family, and
source.  Each slice receives Wilson confidence intervals for precision, recall,
false-positive rate, accuracy, and coverage, plus denominator gates that mark
sparse slices as exploratory instead of paper-facing claims.  This is deliberately
more conservative than a leaderboard table: the current executable corpus is
small enough that most operator-family and model-family slices are underpowered,
and the framework axis is honestly degenerate because all scored programs are
PyTorch modules.  The mined GitHub corpus contributes only positive-only
sample-size coverage for those same axes; it is not used to inflate measured
precision or recall.
...
than requested chunks, zero-size split sections, exact concat reconstruction of
partitions, broadcasting of attention masks, padding tuple/rank corner cases,
compressed sparse block constraints, named-axis alignment/refinement, and
duplicate-name rejection.  The same oracle style now covers embedding-bag corner
...
failure that real code could contain.  The clean-model mutation harness therefore
admits only mutants whose parent executes under eager PyTorch and whose mutant
raises under eager PyTorch, then automatically minimizes the admitted source with
deterministic line-level delta debugging.  A deletion is accepted only when the
candidate still instantiates, still raises, and preserves the original exception
class plus message prefix.  The current artifact contains 756 admitted mutants
from 275 validated-clean parents; all are killed by sound mode, none are reported
\safe{}, and 716 minimized reproducers shrink by 2,990 logical lines while
remaining one-line-minimal under this failure-preserving deletion relation.
...
\subsection{Latency and deployment budget protocol}
...
The Step-256 Pareto harness adds the missing joint view: source models are swept
by size and CEGAR budget, operator occurrences are checked against the live
coverage census, \unknown{} outcomes are counted as abstentions with reasons, and
the non-dominated budget frontier is computed from deterministic structural work
and quality axes.  A separate volatile companion reports latency only as a ratio
to an anchor verification measured in the same run, so the paper can discuss
hardware-normalized cost without committing raw host-specific timings.

Deployment adds a second budget surface: the verifier must be cheap enough to
run both before and after graph capture.  The deployment budget harness measures
real FX pre-export/pre-compile checks, \texttt{torch.export} post-export checks,
and \texttt{torch.dynamo} post-compile checks when the interpreter supports
\texttt{torch.compile}; unsupported compile backends become explicit skips
rather than false passes.  Its committed artifact stores deterministic
latency/memory budgets plus non-dominated per-backend Pareto curves, while the
live gate measures wall-clock time and verifier-stage memory on real
\texttt{nn.Module}s.  A companion deployment gallery stresses architectural
diversity instead of cost: compact ResNet, ViT, Llama-style, diffusion U-Net,
recommender, and speech blocks execute eagerly, then pass both the FX pre-export
gate and the \texttt{torch.export} post-capture gate in supported environments.
The deployment dashboard adds the release-management view: quantization, export,
compile, and distributed rows are tracked per release, and supported rows are
ratcheted so a pass cannot silently become a skip/fail or disappear from the
...
runtime-proven, automatically minimized, and killed without \safe{} survivors. \\
\hline
...
\hline
Deployment budget gate & Release-host regressions in export/compile checks &
Pre/post export and compile gates stay within latency and memory budgets. \\
\hline
Deployment gallery & Architecture-specific export blind spots &
Representative ResNet, ViT, Llama-style, diffusion, recommender, and speech
blocks pass the same pre/post export gates. \\
\hline
Signed certificate replay & Stale or unauthenticated CI safety claims &
MAC-verified certificates structurally replay and invalidate on source, config,
dependency, or soundness-contract drift without solver work. \\
\hline
...
scanning; \texttt{tensorguard sarif-trends} then consumes ordered SARIF snapshots
and reports open, closed, recurrent, and per-rule release deltas using Code
Scanning fingerprints as a counted multiset, so repeated findings are not
collapsed.  Baseline/suppression support lets a legacy repository adopt the tool
...
diagnostics.  For high-assurance release gates, the same SAFE result can be
packaged as a signed certificate: the build signs a canonical payload containing
the top-level safety claim, proof replay data, and drift fingerprints; later CI
jobs authenticate it with a shared secret and re-check the proof structure and
current fingerprints without paying the solver cost again.
...
a model, exporting an artifact, or serving a checkpoint.  Hooks and production
adapters cover Lightning \texttt{fit}, HuggingFace Trainer \texttt{train},
Accelerate \texttt{prepare}, Keras Core \texttt{fit}, and Ray Train
\texttt{fit}/train-loop execution, in addition to MLflow, W\&B, and CI systems.
Each adapter verifies the original \texttt{nn.Module} before the framework wraps,
moves, compiles, or trains it, so a shape/device/phase bug appears as one
\texttt{TensorGuardViolation} at the integration boundary.  The value is not
merely a shape checker; it is a common safety layer that can speak JSON, SARIF,
JUnit, or native annotations.
...
relations, ONNX-inexpressible derived dynamic dimensions, AOTInductor input
layouts, dtype/device policies, \texttt{torch.compile} guard coverage,
preserved dynamic guards and package-lowering
denylists, quantization observer calibration,
...
and pipeline activation boundaries.  \tg{}'s deployment modules check these contracts before the
downstream system emits a harder-to-debug compiler, exporter, or serving error.
For \texttt{torch.compile}, \texttt{src/compile\_guard\_analysis.py} makes the
boundary auditable: it extracts \tg{}'s symbolic input and direct layer
constraints, reads Dynamo guard code from \texttt{torch.\_dynamo.explain} when
available, accepts recorded guard objects for deterministic CI, and reports the
static facts that no runtime guard appears to protect.
\texttt{evaluation/deployment\_budgets.py} turns those checks into a release
budget: FX runs before export and compile, \texttt{torch.export} runs after
export graph capture, and Dynamo runs after compile capture on supported
interpreters.  The manifest records per-backend latency/memory Pareto curves, and
the live gate fails if any supported row is unsafe or over budget.
\texttt{evaluation/deployment\_gallery.py} complements that budget gate with a
model-family gallery: ResNet, ViT, Llama-style, diffusion U-Net, recommender, and
speech modules are instantiated as real PyTorch code, executed to confirm their
deployment input/output contract, then verified before and after export.  The
committed manifest records the model families, input specs, operator surfaces,
and gate matrix; the live gate proves the gallery still passes on the current
torch stack.
\texttt{evaluation/deployment\_dashboard.py} then aggregates the deployment
release tab across quantization, export, compile, and distributed gates.  Its
baseline blocks supported backend regressions while still reporting
environment-qualified skips for optional runtime features such as Dynamo.
For model serving, \texttt{src/serving\_schema.py} splits a pure
\texttt{verify\_serving\_schema} checker from guarded wrappers for
FastAPI-style callables and TorchServe-style handlers.  The pure gate validates
materialized request, preprocessing, model-output, and response payloads against
named specs with concrete, wildcard, or symbolic dimensions; the guarded path
stops on request or preprocessing-output violations before model invocation.
The regression tests run real \texttt{nn.Module}s and prove that an
NHWC image batch is rejected against an NCHW \texttt{Conv2d} schema while the
model's \texttt{forward} counter remains zero.
For CUDA graph capture, \texttt{src/cuda\_graph\_capture.py} separates the
capture eligibility proof from the capture body.  It records static input
signatures, checks replay shape/dtype/device/stride and optional storage-address
reuse, scans the reachable \texttt{forward} helper closure for tensor factories,
data-dependent shape producers such as \texttt{nonzero} and mask indexing, and
host-synchronizing operations such as \texttt{item()}, then cross-checks the
\texttt{torch.fx} graph against the same denylist.  The guarded helper refuses to
invoke capture on violations, and the tests prove this on real \texttt{nn.Module}
forwards while remaining runnable on CPU-only CI.
For GGUF/llama.cpp, the gate reads the checkpoint tensors and metadata directly:
it validates architecture-prefixed metadata, q/k/v or packed-qkv projections,
rotary-pair dimensions, vocabulary and output projections, and per-quantization
block-size divisibility without needing to run the external converter.

For training resume, \tg{} now treats optimizer state as a first-class checkpoint
contract.  The gate maps live optimizer buffers by \texttt{Parameter} identity,
validates AdamW and fused-AdamW moments against target parameter shapes, checks
PyTorch Adafactor's factored \texttt{row\_var}/\texttt{col\_var} buffers, accepts
legitimate lazy pre-step state, verifies declared ZeRO-style shard coverage, and
rejects shape-incompatible checkpoint loads before \texttt{load\_state\_dict}
mutates the optimizer.  Dtype handling follows PyTorch's resume semantics:
castable checkpoint dtypes are not rejected before load, but nonsensical live
state dtypes remain actionable diagnostics.

For LoRA and PEFT fine-tuning, \tg{} separates three concerns that PyTorch and
PEFT normally discover only during load or inference.  Adapter-only checkpoints
are parsed using the HuggingFace key layout, live PEFT modules are inspected via
their \texttt{lora\_A}/\texttt{lora\_B} dictionaries, and every discovered adapter
is checked against the target base weight for rank agreement, input/output
shape, declared \texttt{target\_modules}, expected merged state, and QLoRA-style
quantized-base placement with floating low-rank matrices.  If only an adapter
state is available, base-relative checks are explicitly reported as skipped
rather than converted into a false proof.
...
audit Lean files.  They can also run the reviewer command manifest in dry-run
mode to see the exact command and output path behind each headline table or
figure before choosing the full byte-identical reproduction command.  The signed
certificate checker is similarly inspectable as a small trusted component: it
imports only the Python standard library, verifies the canonical MAC and drift
hashes, and independently replays the proof-DAG shape that the main TensorGuard
replay path accepts.  This is a different posture from a one-off prototype: the
paper claims are connected to code paths, tests, and artifacts, and unsupported
regions are part of the documented result.

The camera-ready package now includes a generated claim ledger.  The block below
is produced by \texttt{reproducibility/camera\_ready\_paper.py} from
\texttt{reproducibility/paper\_evidence\_index.json} and its indexed evidence
artifacts; CI fails if any number drifts without this paper changing too.
...
\hline
Optimizer-state mismatch & A resumed optimizer buffer has the wrong shape, dtype,
factored Adafactor layout, or shard coverage for the target parameter. & Report
the parameter, state key, expected buffer contract, and whether the problem blocks
checkpoint loading or is only a lazy/master-state warning. \\
\hline
Export/serving precondition & ONNX, AOT, quantization, serving, CUDA graph, or accelerator-specific
...
\texttt{operator\_confidence\_table.json}, then checking the generated
\texttt{formal\_soundness\_appendix.pdf}.  They can run the latest padding
tests with:
\[
