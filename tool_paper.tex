...
The newest formal layer lifts those local facts to whole straight-line modules.
...
transfer succeeds, every intermediate and final tensor in the entire
straight-line module remains well-formed.  The registry surface is synchronized
with the current 136 non-heuristic operator-confidence entries, while complex
operators still rely on their local conformance tests rather than being
over-claimed in the composition proof.
...
artifacts, and an unusually broad reproducibility harness.  The latest SDPA proof work illustrates the project's standard: match PyTorch's
exact edge-case semantics, propagate precision to downstream model checks,
abstain on symbolic or value-dependent uncertainty, and pin the behavior with
Lean plus live-library tests.
...
\hline
Formal artifacts &
Lean proof files, axiom audits, SMT encoding faithfulness checks, generated
einops conformance bridges, and proof certificate infrastructure. \\
\hline
Reproducibility &
