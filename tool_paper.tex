...
shape-inference round trips, optimizer-state resume gates, and artifact scripts that regenerate
...
GGUF/llama.cpp-oriented checks, optimizer-state load/resume preflights,
...
optimizer-state checkpoint compatibility, and deployment schemas.
...
New optimizer-state checks add another resume-only failure class: AdamW and
fused-AdamW moment buffers, Adafactor factored \texttt{row\_var}/\texttt{col\_var}
buffers, lazy pre-step state, and declared ZeRO-style shards are validated before
an incompatible checkpoint can corrupt a resumed run.
...
\texttt{src/gguf\_export.py}, \texttt{src/optimizer\_state\_verify.py},
...
gates, including GGUF/llama.cpp checkpoint preflight checks, AdamW/Adafactor
optimizer-state resume checks, and FSDP2/DTensor
...
and metadata consistency before an external converter is invoked; optimizer-state
gates check AdamW, Adafactor, fused AdamW, and sharded resume buffers before
...
state\_dict tensors, logical quantized descriptors, and real optimizer
state\_dict compatibility for AdamW, Adafactor, fused AdamW, lazy state, sharded
resume buffers, and load-time shape rejection. \\
\hline
