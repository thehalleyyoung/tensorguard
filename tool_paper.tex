...
normalization layers, sparse layouts, distributed wrappers, and deployment
...
functional calls such as \texttt{F.linear}, \texttt{F.interpolate}, and
\texttt{F.scaled\_dot\_product\_attention}.  It records scalar constructor
...
broadcasting, reductions, cat/stack, transpose/permute, squeeze/unsqueeze,
...
advanced indexing, gather/scatter, index select, masked select, take-along-dim,
sort/topk families, sparse COO/CSR/CSC/BSR/BSC layout invariants, FFT/complex
view contracts, named tensor alignment, vmap/functorch batch dimensions,
distribution batch/event/log-prob shapes, grid sampling, affine grids,
multi-head attention, scaled dot-product attention, LoRA adapter compatibility,
training-loop checks, quantization/export safety, distributed sharding, tensor
parallel attention variants, and deployment schemas.

\subsection{Exact Partition Operators}

The latest completed implementation round strengthens \texttt{torch.chunk} and
\texttt{torch.split}.  These operators look simple, but their PyTorch semantics
matter in downstream checks.  \texttt{torch.chunk(x, chunks)} may return fewer
than \texttt{chunks} outputs when the split dimension is positive and smaller
than requested.  For example, chunking a length-5 dimension into 8 chunks
returns five length-1 tensors, not eight tensors with empty tails.  A zero-length
dimension is special: chunking length 0 into 3 chunks returns three empty
tensors.  Integer \texttt{torch.split} produces fixed-size chunks with a
possibly smaller final chunk; section-list \texttt{split} requires the section
sizes to sum exactly to the dimension and permits zero-sized sections.

\tg{} now uses shared PyTorch-faithful calculators for these rules.  Tuple
unpacking records one graph step per output element, so
\texttt{a,b,c = x.chunk(3, dim=-1)} gives \texttt{c} the actual final-chunk
width.  This is essential for catching a downstream \texttt{nn.Linear} that
expects the first chunk's width but receives the smaller final chunk.  The same
rule handles method and functional forms, negative dimensions, empty sections,
and concrete unpack-count mismatches.  When the partition size or output count
depends on a symbolic dimension, the split axis is replaced with a fresh
symbolic dimension while non-split dimensions are preserved.  That preserves
useful downstream checks without inventing a false concrete chunk size.
...
these parameters are static.  Normalization rules check channel or normalized
shape contracts, while phase annotations make train/eval-sensitive modules
diagnostic instead of invisible.
...
Data-dependent operators are treated honestly.  \texttt{masked\_select} and
\texttt{nonzero}-style operations can produce ranks or lengths that depend on
runtime tensor values.  Rather than guessing, \tg{} emits symbolic or unknown
facts and, where supported, reports explicit \unknown{} reasons.  This design
is repeated throughout the codebase: useful exactness where the contract is
shape-determined, conservative abstention where it is not.
...
\texttt{tests/test\_operator\_coverage\_gate.py}.  Tracks supported operators,
confidence tags, and release gates. \\
\hline
Latency and scaling &
...
divergences, gradient detach bugs, invalid sparse compressed layouts, named
tensor alignment failures, incompatible distribution batch/event shapes, export
contracts, quantization observer placement, and distributed sharding mistakes.
...
grid sampling, sparse layouts, complex FFT views, named tensors, distributions,
vmap, and now chunk/split partitioning.  When PyTorch's edge-case behavior is
surprising, as with \texttt{torch.chunk} returning fewer tensors than requested,
the static rule follows PyTorch rather than a simplified textbook semantics.
...
phase, gradient, dtype, and shape transitions. \\
\hline
...
Sparse layout and complex/FFT shape-dtype contracts. \\
\hline
\texttt{src/distributions\_verify.py}, \texttt{src/named\_tensor\_verify.py},
\texttt{src/vmap\_verify.py} &
High-value library contracts beyond ordinary dense layers. \\
\hline
\texttt{src/quant\_export\_checks.py}, \texttt{src/distributed\_verification.py},
\texttt{src/lora\_verification.py} &
Deployment, distributed training, and adapter-compatibility gates. \\
\hline
...
chunk/split work illustrates the project's standard: match PyTorch's exact
edge-case semantics, propagate precision to downstream model checks, abstain
on symbolic uncertainty, and pin the behavior with live-library tests.
...
\hline
Shape algebra &
Matmul, reshape/view, flatten, broadcasting, cat, stack, chunk, split,
transpose, permute, squeeze, unsqueeze, reductions, indexing, convolution,
pooling, embedding, and attention rules. \\
\hline
Device reasoning &
...
\hline
Dtype reasoning &
Promotion/cast checks, autocast boundaries, parameter/input dtype matching,
quantization/export dtype gates, and dtype-related runtime-error prevention. \\
\hline
Phase reasoning &
...
\hline
Sparse tensors &
COO/CSR/CSC/BSR/BSC shape and blocksize invariants, with sparse operation
...
AOTInductor packaging, and deployment gates. \\
\hline
...
Equation parser and output-shape tests for einsum, including diagonals and
ellipsis handling. \\
\hline
...
Dtype propagation, promotion, and boolean/integer safety. \\
\hline
...
Deployment-facing gates for export, opsets, AOT packaging, and quantization. \\
\hline
...
\hline
Stack &
All inputs must have equal rank and equal dimensions; output inserts a new
axis of length equal to the number of tensors. &
Stack tests and downstream shape propagation tests. \\
\hline
Chunk/split &
...
\hline
Indexing and gather &
Gather and take-along-dim return index shapes; index-select replaces the
selected dimension; scatter preserves the input shape; narrow replaces a
dimension with a known length. &
Indexing/gather tests and extended operator tests. \\
\hline
Masked and value-dependent ops &
Masked select and nonzero-like behavior can depend on tensor values; the
verifier uses symbolic dynamic lengths and explicit unknown reasons where
appropriate. &
Masked-select tests and soundness-boundary tests for data-dependent ranks. \\
\hline
Einsum &
Equation parser handles explicit and implicit outputs, ellipses, broadcasting,
and repeated-label diagonal constraints where supported. &
...
and \texttt{out\_dims}; impossible batch contracts are rejected. &
vmap verification tests and live dispatcher checks. \\
\hline
Export gates &
Export, ONNX, AOTInductor, and quantization paths check static preconditions
before invoking downstream compilers. &
ONNX, AOT, quantization, and guarded compile/export tests. \\
\hline
...
masks, compressed sparse block constraints, and named-axis alignment.
...
dtype casts, gradient detaches, invalid sparse metadata, and export-incompatible
...
availability, AOTInductor input layouts, quantization observer placement,
mixed-precision dtype boundaries, TorchServe or FastAPI request schemas, GGUF
or llama.cpp checkpoint shapes, CUDA graph capture assumptions, and
distributed shard boundaries.  \tg{}'s deployment modules check these contracts
before the downstream system emits a harder-to-debug compiler, exporter, or
serving error.
...
the installed PyTorch version wherever possible.  The chunk/split rule is a
model example: intuitive equal partitioning would be wrong, so the rule follows
observed torch behavior.
...
\texttt{operator\_confidence\_table.json}.  They can run the new partition
tests with:
\[
\texttt{python -m pytest -q tests/test\_split\_chunk\_precise.py}.
\]
