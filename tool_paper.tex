...
fold/unfold image-column transforms, embedding, dtype casts, device moves, and stochastic tensor factories whose
...
\texttt{kthvalue}, arg reductions, sparse COO/CSR/CSC/BSR/BSC layout invariants,
FFT/complex view contracts, named tensor alignment, vmap/functorch batch dimensions,
...
The latest completed implementation round strengthens a family that returns
...
these parameters are static.  Fold/unfold rules use the same spatial arithmetic
...
fold/unfold, grid sampling, sparse layouts, complex FFT views, named tensors,
distributions, vmap, chunk/split partitioning, and index-value operators.  When PyTorch's
...
index-value work illustrates the project's standard: match PyTorch's exact
...
pooling, fold/unfold, embedding, and attention rules. \\
\hline
Device reasoning &
...
\texttt{sort}, \texttt{topk}, \texttt{kthvalue}, \texttt{argmax}, and
\texttt{argmin}, including tuple index dtypes and static error cases. \\
\hline
...
indices. &
Indexing/gather tests, index-value precise tests, and extended operator tests. \\
\hline
...
\texttt{operator\_confidence\_table.json}.  They can run the new index-value
tests with:
\[
\texttt{python -m pytest -q tests/test\_index\_value\_ops\_precise.py}.
\]
